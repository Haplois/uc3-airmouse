...
if (process.argv[2] === 'recover') {
  ...
  let controller, nativeUI, lastStatus, lastUIStatus, lastSession;
  const snapshot = new StatusSnapshot(`${stateDir}/status.json`);
  const publish = () => {
    if (!controller) return;
    const state = controller.state();
    const content = JSON.stringify(state);
    if (lastStatus !== content) { snapshot.publish(state); lastStatus = content; }
  ...
  controller = new Controller({ sensor, output, configFile, publish, ownership: new OwnershipRequests({ dir: process.env.AIRMOUSE_RUN_DIR ?? '/run/airmouse' }) });
  ...
  const shutdown = async () => {
    ...
    nativeUI?.close();
    launcher.close();
    clearTimeout(controller.releaseTimer); clearTimeout(controller.startupTimer);
