...
static_assert(ARRAY_SIZE(bmi323_acc_gyro_odr) ==
	      ARRAY_SIZE(bmi323_acc_gyro_odrns));
...
static irqreturn_t bmi323_irq_thread_handler(int irq, void *private)
{
	...
	if (status & (BMI323_STATUS_MOTION_MSK | BMI323_STATUS_TAP_MSK)) {
		input_report_key(data->wake_input, KEY_WAKEUP, 0);
		input_report_key(data->wake_input, KEY_WAKEUP, 1);
		input_sync(data->wake_input);
	}
			  ...
			  enum bmi323_sensor_type sensor, int *odr, int *uodr)
{
	...
	if (!odr_raw || odr_raw > ARRAY_SIZE(bmi323_acc_gyro_odr))
		return -EINVAL;
				       ...
				       int odr_index)
{
	...
	if (bmi323_acc_gyro_odr[odr_index][0] > 800)
		mode = ACC_GYRO_MODE_HIGH_PERF;
	else if (bmi323_acc_gyro_odr[odr_index][0] > 25)
...
int bmi323_core_probe(struct device *dev)
{
	...
	data->wake_input = devm_input_allocate_device(dev);
	if (!data->wake_input)
		return -ENOMEM;
	data->wake_input->name = "bmi323";
	input_set_capability(data->wake_input, EV_KEY, KEY_WAKEUP);
	ret = input_register_device(data->wake_input);
	if (ret)
		return ret;
